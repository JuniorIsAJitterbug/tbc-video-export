...
class TestWrappersPALSvideo(unittest.TestCase):
    ...
    def test_videosystem(self) -> None:  # noqa: D102
        opts = self.parse_opts([self.path, "pal_svideo"])
    ...
    def test_decoder_default_luma_decoder_pal_svideo(self) -> None:  # noqa: D102
        opts = self.parse_opts([self.path, "pal_svideo", "--threads", "4"])
        self.files = FileHelper(opts, self.config)
        ...
        self.assertEqual(
            ...
            " ".join(
                [
                    ...
                    "-i PIPE_IN",
                    "-filter_complex",
                    "[1:v]format=yuv422p10le[chroma];[0:v][chroma]mergeplanes=map1s=1:map1p=1:map2s=1:map2p=2:format=yuv422p10le,setfield=tff[v_output]",
                    "-map [v_output]",
