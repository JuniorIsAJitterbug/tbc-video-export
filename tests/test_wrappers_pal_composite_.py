...
class TestWrappersPALComposite(unittest.TestCase):
    """Test wrapper opts for PAL Composite (CVBS) TBC."""

    def setUp(self) -> None:  # noqa: D102
        self.path = Path.joinpath(Path(__file__).parent, "files", "pal_composite")
    ...
    def test_videosystem_pal(self) -> None:  # noqa: D102
        _, opts = self.parse_opts(
            [str(self.path), "pal_composite", "--input-tbc-json", str(self.tbc_json)]
        )
        self.files = FileHelper(opts, self.config)
        self.assertTrue(self.files.tbc_json.video_system, VideoSystem.PAL)

    def test_decoder_default_decoder_opts_pal(self) -> None:  # noqa: D102
    ...
    def test_default_ffmpeg_opts_cvbs_pal(self) -> None:
        """Test FFmpeg opts for Composite (CVBS) PAL TBC.

        General
        Unique ID                                : 79377938116607118702738816603513043574 (0x3BB7A281E03B667E4EC2CEA557863A76)
        Complete name                            : pal_composite.mkv
        Format                                   : Matroska
        ...
        self.assertEqual(
            ...
            " ".join(
                [
                    ...
                    "-i PIPE_IN",
                    "-filter_complex",
                    "[0:v]setfield=tff,format=yuv422p10le[v_output]",
                    "-map [v_output]",
    ...
    def test_ffmpeg_opts_pal_cvbs_luma(self) -> None:
        """Test FFmpeg opts for luma only Composite (CVBS) PAL TBC.

        General
        Unique ID                                : 315014768533646516117785922204933804777 (0xECFDA3376A3FF3E9CD7B7B745F5FD2E9)
        Complete name                            : pal_composite_luma.mkv
        Format                                   : Matroska
        Format version                           : Version 4
        File size                                : 183 KiB
        Duration                                 : 40 ms
        ...
        self.assertEqual(
            ...
            " ".join(
                [
                    ...
                    "-i PIPE_IN",
                    "-filter_complex",
                    "[0:v]extractplanes=y,setfield=tff,format=gray16le[v_output]",
                    "-map [v_output]",
                    "-timecode 00:00:00:00",
                    "-framerate pal",
                    "-color_range tv",
    ...
    def test_ffmpeg_opts_pal_cvbs_luma_4fsc(self) -> None:
        """Test FFmpeg opts for luma 4FSC Composite (CVBS) PAL TBC.

        General
        Unique ID                                : 167305488139844949423045550293809774562 (0x7DDDDD6D28B8407FAB29E680DB4E3FE2)
        Complete name                            : pal_composite_luma_4fsc.mkv
        Format                                   : Matroska
        Format version                           : Version 4
        File size                                : 338 KiB
        Duration                                 : 40 ms
        ...
        self.assertEqual(
            ...
            " ".join(
                [
                    ...
                    "-filter_complex",
                    "[0:v]il=l=i:c=i,setfield=tff,format=gray16le[v_output]",
                    "-map [v_output]",
                    "-timecode 00:00:00:00",
                    "-framerate pal",
                    "-color_range tv",
