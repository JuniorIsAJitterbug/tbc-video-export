    ...
    def _generate_profile(self, profile_name: str) -> list[Profile]:
        try:
            ...
            if isinstance(profile_data["video_profile"], list):
                video_profiles = [
                    ProfileVideo(json_profile)
                    for json_profile in self._data["video_profiles"]
                    for profile_name in profile_data["video_profile"]
            ...
            audio_profile = next(
                (
                    ProfileAudio(json_profile)
                    for json_profile in self._data["audio_profiles"]
                    if json_profile["name"] == profile_data["audio_profile"]
                ),
                None,
            )
            ...
            for video_profile in video_profiles:
                ...
                if (override := video_profile.filter_profiles_override) is not None:
                    profile.filter_profiles = [
                        ProfileFilter(json_profile)
                        for json_profile in self._data["filter_profiles"]
                        for filter_name in override
                        if json_profile["name"] == filter_name
                    ]
